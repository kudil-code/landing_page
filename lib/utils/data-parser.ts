...
export function parseXMLTenderData(xmlData: unknown): TenderCreate[] {
  const tenders: TenderCreate[] = [];
  
  if (xmlData && typeof xmlData === 'object' && 'data_paket_lpse' in xmlData) {
    const data = xmlData as { data_paket_lpse: { paket: unknown } };
    if (data.data_paket_lpse && data.data_paket_lpse.paket) {
      const pakets = Array.isArray(data.data_paket_lpse.paket) 
        ? data.data_paket_lpse.paket 
        : [data.data_paket_lpse.paket];

      for (const paket of pakets) {
        if (paket && typeof paket === 'object') {
          const p = paket as Record<string, unknown>;
          const tender: TenderCreate = {
            kode_paket: (p.kode_paket as string) || '',
            nama_paket: (p.nama_paket as string) || '',
            nilai_paket: parseCurrency((p.nilai_paket as string) || '0'),
            nilai_paket_formatted: (p.nilai_paket as string) || 'Rp. 0,00',
            tanggal_pembuatan: parseIndonesianDate((p.tanggal_pembuatan as string) || ''),
            tanggal_pembuatan_original: (p.tanggal_pembuatan as string) || '',
            lokasi_pekerjaan: (p.lokasi_pekerjaan as string) || '',
            satuan_kerja: (p.satuan_kerja as string) || '',
            jenis_pengadaan: (p.jenis_pengadaan as string) || '',
            status: 'active'
          };
...
export async function parseXMLTenderDataWithHtml(xmlData: unknown, htmlFilesPath?: string): Promise<TenderCreate[]> {
  const tenders: TenderCreate[] = [];
    ...
    if (data.data_paket_lpse && data.data_paket_lpse.paket) {
      ...
      for (const paket of pakets) {
        if (paket && typeof paket === 'object') {
          ...
          if (htmlFilesPath && tender.kode_paket) {
            try {
              ...
              console.warn(`Failed to find HTML file for tender ${tender.kode_paket}:`, error);
            }
          }
...
export function tenderToCSV(tenders: unknown[]): string {
